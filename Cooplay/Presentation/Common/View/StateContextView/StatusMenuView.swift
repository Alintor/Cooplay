...
class StatusMenuView: UIView {
    ...
    enum MenuType {
        ...
        case statuses(type: StatusMenuItem.StatusesType, event: Event, actionHandler: ((_ status: User.Status) -> Void)?)
        ...
        var items: [MenuItem] {
            switch self {
            case .statuses(let type, let event, let actionHandler):
                return type.items.map { StatusMenuItem(status: $0, event: event, actionHandler: actionHandler) }
